-- Drop and recreate the unified feed RPC to include event cover image
DROP FUNCTION IF EXISTS get_unified_feed(uuid,text,text,double precision,double precision,integer,integer,integer);
...
AS $$
  ...
  SELECT
    ...
    r.total_weight_kg,
    r.pollution_counts,
    (SELECT ri.storage_path FROM report_images ri WHERE ri.report_id = r.id AND ri.is_primary = true LIMIT 1) as image_url,
    (SELECT ai.scene_labels FROM ai_analysis ai WHERE ai.report_id = r.id LIMIT 1) as scene_labels,
    (SELECT COUNT(*) FROM report_thanks rt WHERE rt.report_id = r.id) as thanks_count,
    (p_user_id IS NOT NULL AND EXISTS(SELECT 1 FROM report_thanks rt WHERE rt.report_id = r.id AND rt.user_id = p_user_id)) as user_has_thanked,
    ...
    NULL::TEXT as event_cover_image_url
  FROM reports r
  LEFT JOIN profiles p ON r.user_id = p.id
  CROSS JOIN user_point up
  WHERE
    (p_radius_km IS NULL OR up.point IS NULL OR ST_DWithin(r.location::geography, up.point, p_radius_km * 1000))
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM event_participants ep WHERE ep.event_id = e.id AND ep.status IN ('joined', 'checked_in')) as event_attendee_count,
    (p_user_id IS NOT NULL AND EXISTS(SELECT 1 FROM event_participants ep WHERE ep.event_id = e.id AND ep.user_id = p_user_id AND ep.status IN ('joined', 'checked_in'))) as user_has_joined,
  ...
  WHERE
    ...
    AND (p_radius_km IS NULL OR up.point IS NULL OR ST_DWithin(e.location::geography, up.point, p_radius_km * 1000))
